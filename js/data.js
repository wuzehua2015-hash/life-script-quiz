/**
 * 人生剧本测试 v2.0 - 数据文件（重构版）
 * 包含80个影视/文学角色库、12种原型、匹配算法
 * 重构日期: 2026-02-25
 * 重构内容:
 * 1. 修正角色原型分类（甄嬛、林黛玉、祥子等）
 * 2. 统一cast描述规范，符合角色实际家庭背景
 * 3. 修正四维属性匹配
 * 4. 消除角色重复问题
 * 5. 新增5个角色保持80个总数
 */
...
const CHARACTER_LIBRARY = {
    // 孤勇者 (Lone Hero) - 7个角色
    // 核心特征: drive=achievement, world=battle, self=perfection, time=chasing
    lone_hero: [
        // 国产剧
    ...
    // 讨好者 (Pleaser) - 6个角色
    // 核心特征: drive=relationship, world=victim, self=inferiority, time=fate
    pleaser: [
        // 国产剧
    ...
    // 隐士 (Hermit) - 8个角色
    // 核心特征: drive=security, world=detachment, self=inferiority/lost, time=stagnation
    hermit: [
        // 国产剧
    ...
    // 控制狂 (Controller) - 8个角色
    // 核心特征: drive=achievement, world=control, self=perfection, time=chasing
    controller: [
        // 国产剧
    ...
    // 表演者 (Performer) - 7个角色
    // 核心特征: drive=unique, world=narcissism, self=perfection, time=exploration
    performer: [
        // 国产剧
    ...
    // 拯救者 (Savior) - 7个角色
    // 核心特征: drive=service, world=cooperation, self=inferiority, time=creation
    savior: [
        // 国产剧
    ...
    // 漫游者 (Wanderer) - 7个角色
    // 核心特征: drive=unique, world=detachment, self=lost, time=exploration
    wanderer: [
        // 国产剧
    ...
    // 战士 (Warrior) - 8个角色
    // 核心特征: drive=achievement, world=battle, self=authenticity, time=creation
    warrior: [
        // 国产剧
    ...
    // 治愈者 (Healer) - 7个角色
    // 核心特征: drive=relationship, world=cooperation, self=authenticity, time=creation
    healer: [
        // 国产剧
    ...
    // 观察者 (Observer) - 7个角色
    // 核心特征: drive=security, world=detachment, self=authenticity, time=exploration
    observer: [
        // 国产剧
    ...
    // 觉醒者 (Awakened) - 7个角色
    // 核心特征: self=authenticity, world=cooperation, time=creation
    awakened: [
        // 国产剧
...
// rebuild v2.1 - 80 characters library (refactored 2026-02-25)
// Changes:
// 1. Fixed 甄嬛 classification (moved from lone_hero to controller)
// 2. Fixed 林黛玉 classification (moved from pleaser to hermit)
// 3. Fixed 祥子 classification (moved from pleaser to victim)
// 4. Fixed 魏璎珞 classification (moved from performer to controller)
// 5. Fixed Tony Stark classification (moved from lone_hero to performer)
// 6. Fixed Arya Stark classification (moved from wanderer to warrior)
// 7. Fixed 王漫妮 classification (moved from wanderer to performer)
// 8. Added 5 new characters: 闰土, 孙悟空, 鲁智深, 吴用
// 9. Removed duplicate entries and unified character descriptions
// 10. Updated cast descriptions to match actual character backgrounds
